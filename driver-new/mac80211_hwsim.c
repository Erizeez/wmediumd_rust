...
static int hwsim_clone_frame(struct sk_buff *skb_2,
							 const u8 *dst, u32 rate_idx,
							 u32 freq, u32 signal)
{
...
static int hwsim_rx_nl(struct sk_buff *skb_2,
					   struct genl_info *info)
{
	...
	signal = nla_get_u32(info->attrs[HWSIM_ATTR_SIGNAL]);

	data2 = get_hwsim_data_ref_from_addr(src);
	if (!data2)
		goto out;

	// printk(KERN_INFO "recv1");
	...
	hwsim_clone_frame(skb, dst, rate_idx, freq, signal);
